...
resampletables = 1; % Set this to 1 to resample table identies 
...
for it = 1:NITS
    ...
    if resampletables
        for j = 1:J
            for k = 1:N_tables(j)
                % Remove this table from the top-level component
                thisTop = tables_to_top{j}(k);
                N_tables_per_top(thisTop) = N_tables_per_top(thisTop) - 1;
                pos = find(Z{j}==k);
                top_N(thisTop) = top_N(thisTop) - length(pos);
                top_S(thisTop) = top_S(thisTop) - sum(x{j}(pos));
                if N_tables_per_top(thisTop) == 0
                    % Delete the top level component
                    top_N(thisTop) = [];
                    top_S(thisTop) = [];
                    N_top = N_top - 1;
                    N_tables_per_top(thisTop) = [];
                    for jp = 1:J
                        tpos = find(tables_to_top{jp} > thisTop);
                        tables_to_top{jp}(tpos) = tables_to_top{jp}(tpos) - 1;
                    end
                end
                Like = zeros(1,N_top+1);
                for i = 1:N_top
                    Like(i) = 0;
                    tempN = top_N(i);
                    tempS = top_S(i);
                    for p = 1:length(pos)
                        Like(i) = Like(i) + clusterlike(x{j}(pos(p)),tempN,tempS,base_mean,base_prec,obs_prec);
                        tempN = tempN + 1;
                        tempS = tempS + x{j}(pos(p));
                    end
                end
                Like(end) = 0;
                tempN = 0;
                tempS = 0;
                for p = 1:length(pos)
                    Like(end) = Like(end) + clusterlike(x{j}(pos(p)),tempN,tempS,base_mean,base_prec,obs_prec);
                    tempN = tempN + 1;
                    tempS = tempS + x{j}(pos(p));
                end
                Prior = [N_tables_per_top gamma];
                Post = Like + Prior;
                Post = exp(Post - max(Post));
                Post = Post./sum(Post);
                newTop = find(rand<=cumsum(Post),1);
                if newTop > N_top
                    N_top = N_top + 1;
                    tables_to_top{j}(k) = N_top;
                    N_tables_per_top(1,N_top) = 1;
                    top_N(1,N_top) = length(pos);
                    top_S(1,N_top) = sum(x{j}(pos));
                else
                   tables_to_top{j}(k) = newTop;
                   N_tables_per_top(newTop) = N_tables_per_top(newTop) + 1;
                   top_N(newTop) = top_N(newTop) + length(pos);
                   top_S(newTop) = top_S(newTop) + sum(x{j}(pos));
                end
            end
        end
    end
    ...
    figure(1);
    hold on
    for i = 1:length(postmean)
        plot(it,postmean(i),'k.');
    end
    figure(2)
    hist(allN_top,[1:10]);
    drawnow
...
% This checks my bookkeeping is correct!
J = length(x);
